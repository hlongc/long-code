...
Sensitive File 规则：
53. 不要主动读取 .env、.npmrc、私钥、证书、token 配置等敏感文件。
54. 如果用户明确要求读取敏感文件，必须等待权限确认。
55. 不要在最终回答中完整输出密钥、token、私钥内容；如需说明，只能概括是否存在相关配置。
56. 对私钥文件如 id_rsa、id_ed25519，应默认拒绝访问。
`;